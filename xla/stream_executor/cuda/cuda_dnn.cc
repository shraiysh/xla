...
class CudnnGraphRunner<void(Args...)> : public dnn::OpRunner<void(Args...)> {
 ...
 public:
  std::string ToString() const override { return graph_->print(); }
  ...
  absl::Status operator()(Stream* stream, dnn::ProfileResult* profile_result,
    ...
    variant_pack[ALPHA_SCALE_ID] =
        const_cast<void*>(attn_scale_val_.ToVoidPointer());
  ...
  static absl::StatusOr<CudnnGraphRunner> Create(
      GpuExecutor* parent, CudnnAccess* cudnn, std::shared_ptr<Graph> graph,
      bool has_dropout, int64_t dropout_rng_seed, int64_t dropout_rng_offset,
      ScalingParam attn_scale_val,
      std::vector<std::optional<CudnnfMHAUid>> uids) {
    return {{parent, cudnn, graph, has_dropout, dropout_rng_seed,
             dropout_rng_offset, attn_scale_val, uids}};
  }

 private:
  CudnnGraphRunner(GpuExecutor* parent, CudnnAccess* cudnn,
                   std::shared_ptr<Graph> graph, bool has_dropout,
                   int64_t dropout_rng_seed, int64_t dropout_rng_offset,
                   ScalingParam attn_scale_val,
                   std::vector<std::optional<CudnnfMHAUid>> uids)
      : parent_(parent),
        ...
        attn_scale_val_(attn_scale_val),
        uids_(uids) {}
  GpuExecutor* parent_;
  CudnnAccess* cudnn_;
  ...
  ScalingParam attn_scale_val_;
  std::vector<std::optional<CudnnfMHAUid>> uids_;
};
